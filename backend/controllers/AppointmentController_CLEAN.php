...
class AppointmentController {
    ...
    public function create() {
        try {
            ...
            if ($user['role'] === ROLE_PATIENT) {
                ...
                // Un médecin crée un rendez-vous - récupère automatiquement son ID
    ...
    public function createAppointmentRequest() {
        try {
            ...
            $appointmentDate = $input['appointment_date'];
            $appointmentType = $input['appointment_type'] ?? 'consultation';
            $reason = $input['reason_for_appointment'] ?? '';
            $notes = $input['notes'] ?? '';
    ...
    private function createNotification($userId, $type, $title, $message, $patientId = null, $doctorId = null, $appointmentId = null) {
        try {
            $stmt = $this->db->prepare(
                'INSERT INTO notifications (user_id, type, title, message, patient_id, doctor_id, appointment_id, created_at, is_read)
                 VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)'
            );
            ...
            $stmt->bind_param('isssiiisi', $userId, $type, $title, $message, $patientId, $doctorId, $appointmentId, $createdAt, $isRead);
            $stmt->execute();
            $stmt->close();
